...
const AllRooms = () => {
  ...
  const {rooms,navigate,currency} = useAppContext();
    ...
    //Fuction to check if a room meets the selected room type

    const matchesRoomType = (room) =>{
      return selectedFilters.roomType.length === 0 || selectedFilters.roomType.includes(room.roomType);
    }

          //Fuction to check if a room meets the selected price range
 ...
 // Function to sort rooms based on selected sorting option
   ...
   // filter and sort rooms based on selected filters and sorting option
      ...
      <div>
        ...
        <div key={room._id} className="flex flex-col md:flex-row items-start py-10 gap-6 border-gray-300 last:pb-30 last:border-0">
            ...
            <div className="md:w-1/2 flex flex-col gap-2">
                ...
                <p className="text-3xl font-medium text-gray-700">${room.pricePerNight} /night</p>
            </div>
        </div> 
        ))}
      </div>
